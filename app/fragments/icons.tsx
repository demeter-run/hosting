export function LogoHover(props: { className?: string }) {
    return (
        <svg className={props.className} viewBox="0 0 5229 1415" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
                d="M1818.06 1037H1599.98V369.6H1818.06C2008.88 369.6 2140.48 482.4 2140.48 690.14V716.46C2140.48 922.32 2010.76 1037 1818.06 1037ZM1679.88 439.16V967.44H1811.48C1962.82 967.44 2057.76 887.54 2057.76 716.46V690.14C2057.76 520.94 1963.76 439.16 1809.6 439.16H1679.88ZM2453.14 1048.28C2307.44 1048.28 2223.78 947.7 2223.78 804.82V786.96C2223.78 643.14 2309.32 543.5 2452.2 543.5C2605.42 543.5 2677.8 648.78 2671.22 810.46H2300.86C2302.74 922.32 2359.14 984.36 2453.14 984.36C2529.28 984.36 2576.28 949.58 2592.26 887.54H2668.4C2644.9 989.06 2567.82 1048.28 2453.14 1048.28ZM2302.74 749.36H2592.26C2584.74 660.06 2537.74 607.42 2452.2 607.42C2365.72 607.42 2313.08 657.24 2302.74 749.36ZM2843.12 1037H2767.92V554.78H2841.24L2842.18 630.92C2868.5 582.98 2918.32 543.5 2994.46 543.5C3067.78 543.5 3116.66 578.28 3139.22 639.38C3173.06 578.28 3227.58 543.5 3299.96 543.5C3406.18 543.5 3459.76 611.18 3459.76 723.04V1037H3384.56V739.02C3384.56 652.54 3351.66 608.36 3278.34 608.36C3198.44 608.36 3151.44 668.52 3151.44 764.4V1037H3076.24V739.02C3076.24 653.48 3045.22 608.36 2971.9 608.36C2892 608.36 2843.12 668.52 2843.12 766.28V1037ZM3786.47 1048.28C3640.77 1048.28 3557.11 947.7 3557.11 804.82V786.96C3557.11 643.14 3642.65 543.5 3785.53 543.5C3938.75 543.5 4011.13 648.78 4004.55 810.46H3634.19C3636.07 922.32 3692.47 984.36 3786.47 984.36C3862.61 984.36 3909.61 949.58 3925.59 887.54H4001.73C3978.23 989.06 3901.15 1048.28 3786.47 1048.28ZM3636.07 749.36H3925.59C3918.07 660.06 3871.07 607.42 3785.53 607.42C3699.05 607.42 3646.41 657.24 3636.07 749.36ZM4287.64 1048.28C4191.76 1048.28 4141 1004.1 4141 915.74V614.94H4044.18V554.78H4141V406.26H4216.2V554.78H4354.38V614.94H4216.2V905.4C4216.2 958.98 4234.06 984.36 4297.04 984.36C4315.84 984.36 4342.16 979.66 4358.14 974.96V1038.88C4338.4 1043.58 4308.32 1048.28 4287.64 1048.28ZM4650.72 1048.28C4505.02 1048.28 4421.36 947.7 4421.36 804.82V786.96C4421.36 643.14 4506.9 543.5 4649.78 543.5C4803 543.5 4875.38 648.78 4868.8 810.46H4498.44C4500.32 922.32 4556.72 984.36 4650.72 984.36C4726.86 984.36 4773.86 949.58 4789.84 887.54H4865.98C4842.48 989.06 4765.4 1048.28 4650.72 1048.28ZM4500.32 749.36H4789.84C4782.32 660.06 4735.32 607.42 4649.78 607.42C4563.3 607.42 4510.66 657.24 4500.32 749.36ZM5040.7 1037H4965.5V554.78H5038.82L5039.76 647.84C5068.9 581.1 5122.48 543.5 5189.22 543.5C5196.74 543.5 5205.2 543.5 5214.6 545.38V620.58C5204.26 619.64 5197.68 619.64 5187.34 619.64C5099.92 619.64 5040.7 673.22 5040.7 778.5V1037Z"
                fill="#000"
                className="group-hover:fill-accent2 transition-all"
            />
            <path
                d="M17.6899 363.3V1051.36L613.57 1395.38L1209.44 1051.36V363.3L613.57 19.28L17.6899 363.3Z"
                stroke="#000"
                strokeWidth="33.39"
                strokeMiterlimit="10"
                className="group-hover:stroke-accent2 transition-all"
            />
            <path
                d="M621.82 19.62L324.31 193.77L920.48 534.1L919.7 880.63L621.82 1052.6L620.72 363.03L323.79 535.6V1224.62L621.82 1395.72L1217.69 1051.69V363.64L621.82 19.62Z"
                fill="#000"
                className="group-hover:fill-accent2 transition-all"
            />
        </svg>
    );
}

export function BoltIcon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
            <path
                fillRule="evenodd"
                d="M14.615 1.595a.75.75 0 01.359.852L12.982 9.75h7.268a.75.75 0 01.548 1.262l-10.5 11.25a.75.75 0 01-1.272-.71l1.992-7.302H3.75a.75.75 0 01-.548-1.262l10.5-11.25a.75.75 0 01.913-.143z"
                clipRule="evenodd"
            />
        </svg>
    );
}

export function LogIcon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 360 360" fill="currentColor">
            <g id="XMLID_227_">
                <path
                    id="XMLID_228_"
                    d="M344.259,126.683h-48.518V77.834L212.719,0H15.741v360h280v-50.59h48.518V126.683z M314.259,279.41h-210
  V156.683h210V279.41z"
                />
                <polygon
                    id="XMLID_231_"
                    points="170.892,234.942 139.035,234.942 139.035,183.11 126.892,183.11 126.892,245.59 170.892,245.59 	
  "
                />
                <path
                    id="XMLID_232_"
                    d="M178.943,236.218c2.669,2.963,5.866,5.339,9.593,7.128c3.725,1.79,7.876,2.684,12.451,2.684
  c4.4,0,8.477-0.865,12.232-2.596c3.754-1.73,6.98-4.048,9.68-6.952c2.698-2.904,4.811-6.248,6.336-10.032
  c1.525-3.784,2.288-7.758,2.288-11.924c0-3.93-0.734-7.802-2.199-11.616c-1.468-3.813-3.521-7.216-6.16-10.208
  c-2.641-2.992-5.823-5.412-9.548-7.26c-3.727-1.848-7.848-2.772-12.364-2.772c-4.4,0-8.479,0.866-12.232,2.596
  c-3.755,1.732-6.996,4.064-9.724,6.996c-2.729,2.935-4.87,6.307-6.424,10.12c-1.556,3.814-2.332,7.803-2.332,11.968
  c0,3.991,0.733,7.892,2.2,11.704C174.205,229.868,176.273,233.256,178.943,236.218z M184.048,206.738
  c0.792-2.493,1.949-4.722,3.476-6.688c1.525-1.965,3.433-3.549,5.721-4.752c2.288-1.202,4.898-1.804,7.832-1.804
  c2.815,0,5.352,0.572,7.611,1.716c2.258,1.144,4.165,2.684,5.721,4.62c1.554,1.936,2.757,4.166,3.607,6.688
  c0.85,2.523,1.276,5.134,1.276,7.831c0,2.583-0.397,5.119-1.189,7.612c-0.792,2.494-1.966,4.738-3.52,6.732
  c-1.556,1.995-3.463,3.594-5.721,4.796c-2.259,1.203-4.854,1.804-7.787,1.804c-2.876,0-5.442-0.586-7.7-1.76
  c-2.26-1.173-4.166-2.728-5.72-4.664c-1.556-1.936-2.743-4.165-3.564-6.688c-0.822-2.521-1.232-5.133-1.232-7.832
  C182.859,211.769,183.256,209.232,184.048,206.738z"
                />
                <path
                    id="XMLID_236_"
                    d="M245.823,236.79c2.669,2.875,5.809,5.134,9.416,6.776c3.608,1.643,7.465,2.464,11.572,2.464
  c6.629,0,12.436-2.493,17.424-7.48v7.04h10.032v-32.12h-22.616v8.888h12.584v4.4c-4.87,5.691-10.326,8.536-16.368,8.536
  c-2.64,0-5.09-0.542-7.348-1.628c-2.259-1.085-4.21-2.566-5.852-4.444c-1.644-1.877-2.935-4.092-3.872-6.644
  c-0.939-2.552-1.408-5.293-1.408-8.228c0-2.816,0.425-5.485,1.276-8.008c0.85-2.521,2.053-4.752,3.607-6.688
  c1.554-1.936,3.432-3.461,5.632-4.576c2.2-1.114,4.62-1.672,7.261-1.672c3.344,0,6.438,0.822,9.283,2.464
  c2.845,1.644,5.061,4.02,6.645,7.128l9.064-6.688c-2.111-4.165-5.266-7.48-9.46-9.944c-4.195-2.464-9.226-3.696-15.092-3.696
  c-4.342,0-8.36,0.836-12.056,2.508c-3.696,1.672-6.908,3.931-9.637,6.776c-2.728,2.847-4.87,6.16-6.424,9.944
  c-1.555,3.784-2.332,7.818-2.332,12.1c0,4.518,0.777,8.727,2.332,12.628C241.041,230.528,243.153,233.916,245.823,236.79z"
                />
            </g>
        </svg>
    );
}

export function CurrencyDollarIcon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
            <path d="M10.464 8.746c.227-.18.497-.311.786-.394v2.795a2.252 2.252 0 0 1-.786-.393c-.394-.313-.546-.681-.546-1.004 0-.323.152-.691.546-1.004ZM12.75 15.662v-2.824c.347.085.664.228.921.421.427.32.579.686.579.991 0 .305-.152.671-.579.991a2.534 2.534 0 0 1-.921.42Z" />
            <path
                fillRule="evenodd"
                d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25ZM12.75 6a.75.75 0 0 0-1.5 0v.816a3.836 3.836 0 0 0-1.72.756c-.712.566-1.112 1.35-1.112 2.178 0 .829.4 1.612 1.113 2.178.502.4 1.102.647 1.719.756v2.978a2.536 2.536 0 0 1-.921-.421l-.879-.66a.75.75 0 0 0-.9 1.2l.879.66c.533.4 1.169.645 1.821.75V18a.75.75 0 0 0 1.5 0v-.81a4.124 4.124 0 0 0 1.821-.749c.745-.559 1.179-1.344 1.179-2.191 0-.847-.434-1.632-1.179-2.191a4.122 4.122 0 0 0-1.821-.75V8.354c.29.082.559.213.786.393l.415.33a.75.75 0 0 0 .933-1.175l-.415-.33a3.836 3.836 0 0 0-1.719-.755V6Z"
                clipRule="evenodd"
            />
        </svg>
    );
}

export function Bars3Icon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
            <path
                fillRule="evenodd"
                d="M3 6.75A.75.75 0 0 1 3.75 6h16.5a.75.75 0 0 1 0 1.5H3.75A.75.75 0 0 1 3 6.75ZM3 12a.75.75 0 0 1 .75-.75h16.5a.75.75 0 0 1 0 1.5H3.75A.75.75 0 0 1 3 12Zm0 5.25a.75.75 0 0 1 .75-.75h16.5a.75.75 0 0 1 0 1.5H3.75a.75.75 0 0 1-.75-.75Z"
                clipRule="evenodd"
            />
        </svg>
    );
}

export function GlobeAltIcon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
            <path d="M21.721 12.752a9.711 9.711 0 0 0-.945-5.003 12.754 12.754 0 0 1-4.339 2.708 18.991 18.991 0 0 1-.214 4.772 17.165 17.165 0 0 0 5.498-2.477ZM14.634 15.55a17.324 17.324 0 0 0 .332-4.647c-.952.227-1.945.347-2.966.347-1.021 0-2.014-.12-2.966-.347a17.515 17.515 0 0 0 .332 4.647 17.385 17.385 0 0 0 5.268 0ZM9.772 17.119a18.963 18.963 0 0 0 4.456 0A17.182 17.182 0 0 1 12 21.724a17.18 17.18 0 0 1-2.228-4.605ZM7.777 15.23a18.87 18.87 0 0 1-.214-4.774 12.753 12.753 0 0 1-4.34-2.708 9.711 9.711 0 0 0-.944 5.004 17.165 17.165 0 0 0 5.498 2.477ZM21.356 14.752a9.765 9.765 0 0 1-7.478 6.817 18.64 18.64 0 0 0 1.988-4.718 18.627 18.627 0 0 0 5.49-2.098ZM2.644 14.752c1.682.971 3.53 1.688 5.49 2.099a18.64 18.64 0 0 0 1.988 4.718 9.765 9.765 0 0 1-7.478-6.816ZM13.878 2.43a9.755 9.755 0 0 1 6.116 3.986 11.267 11.267 0 0 1-3.746 2.504 18.63 18.63 0 0 0-2.37-6.49ZM12 2.276a17.152 17.152 0 0 1 2.805 7.121c-.897.23-1.837.353-2.805.353-.968 0-1.908-.122-2.805-.353A17.151 17.151 0 0 1 12 2.276ZM10.122 2.43a18.629 18.629 0 0 0-2.37 6.49 11.266 11.266 0 0 1-3.746-2.504 9.754 9.754 0 0 1 6.116-3.985Z" />
        </svg>
    );
}

export function GasIcon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="42.65 63.25 77.3 84.25">
            <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M101.25 133.22C101.25 129.5 101.18 117.57 101.18 114.06C101.18 109.73 100.89 107.59 95.18 107.06C95.09 119.49 95 132.17 95 139H99.15L98.97 147.5L43 147.3L42.65 139H47.32C47.15 124.16 46.62 77.82 46.47 73C46.29 67.25 49.16 63.84 55.99 63.66C62.82 63.48 80.76 63.12 86.68 63.3C92.6 63.48 95.33 65.94 95.33 73.84C95.33 76.93 95.27 88.32 95.2 101.06C105.03 101.98 107.2 106.95 107.2 114.06C107.2 117.55 107.27 130.43 107.27 133.22C107.27 137.01 113.88 137.35 113.88 133.08C113.88 129.98 113.88 102.08 113.88 102.08L105.98 94.71L107.1 83.9L99.2 75.75L103 71.31L119.81 88.06C119.81 88.06 119.95 121.43 119.95 133.06C119.95 144.69 101.25 144.33 101.25 133.22ZM53 73.41C53 73.41 53.31 80.74 53.31 90.23C53.31 104.9 88.77 104.16 88.77 90.08C88.77 82.17 88.93 73.08 88.93 73.08L53 73.41Z"
                fill="#currentColor"
            ></path>
        </svg>
    );
}

export function BranchIcon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor">
            <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M4.75 1.75V1H3.25V1.75V9.09451C1.95608 9.42754 1 10.6021 1 12C1 13.6569 2.34315 15 4 15C5.42051 15 6.61042 14.0127 6.921 12.6869C8.37102 12.4872 9.7262 11.8197 10.773 10.773C11.8197 9.7262 12.4872 8.37102 12.6869 6.921C14.0127 6.61042 15 5.42051 15 4C15 2.34315 13.6569 1 12 1C10.3431 1 9 2.34315 9 4C9 5.37069 9.91924 6.52667 11.1749 6.8851C10.9929 7.94904 10.4857 8.9389 9.71231 9.71231C8.9389 10.4857 7.94904 10.9929 6.8851 11.1749C6.59439 10.1565 5.77903 9.35937 4.75 9.09451V1.75ZM13.5 4C13.5 4.82843 12.8284 5.5 12 5.5C11.1716 5.5 10.5 4.82843 10.5 4C10.5 3.17157 11.1716 2.5 12 2.5C12.8284 2.5 13.5 3.17157 13.5 4ZM4 13.5C4.82843 13.5 5.5 12.8284 5.5 12C5.5 11.1716 4.82843 10.5 4 10.5C3.17157 10.5 2.5 11.1716 2.5 12C2.5 12.8284 3.17157 13.5 4 13.5Z"
                fill="currentColor"
            ></path>
        </svg>
    );
}

export function CommitIcon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
            <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M8 10.5C9.38071 10.5 10.5 9.38071 10.5 8C10.5 6.61929 9.38071 5.5 8 5.5C6.61929 5.5 5.5 6.61929 5.5 8C5.5 9.38071 6.61929 10.5 8 10.5ZM8 12C9.95279 12 11.5787 10.6006 11.9298 8.75H15.25H16V7.25H15.25H11.9298C11.5787 5.39935 9.95279 4 8 4C6.04721 4 4.42125 5.39935 4.0702 7.25H0.75H0V8.75H0.75H4.0702C4.42125 10.6006 6.04721 12 8 12Z"
                fill="currentColor"
            ></path>
        </svg>
    );
}

export function GlobeAmericasIcon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
            <path
                fillRule="evenodd"
                d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25ZM6.262 6.072a8.25 8.25 0 1 0 10.562-.766 4.5 4.5 0 0 1-1.318 1.357L14.25 7.5l.165.33a.809.809 0 0 1-1.086 1.085l-.604-.302a1.125 1.125 0 0 0-1.298.21l-.132.131c-.439.44-.439 1.152 0 1.591l.296.296c.256.257.622.374.98.314l1.17-.195c.323-.054.654.036.905.245l1.33 1.108c.32.267.46.694.358 1.1a8.7 8.7 0 0 1-2.288 4.04l-.723.724a1.125 1.125 0 0 1-1.298.21l-.153-.076a1.125 1.125 0 0 1-.622-1.006v-1.089c0-.298-.119-.585-.33-.796l-1.347-1.347a1.125 1.125 0 0 1-.21-1.298L9.75 12l-1.64-1.64a6 6 0 0 1-1.676-3.257l-.172-1.03Z"
                clipRule="evenodd"
            />
        </svg>
    );
}

export function XCircleIcon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
            <path
                fillRule="evenodd"
                d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25Zm-1.72 6.97a.75.75 0 1 0-1.06 1.06L10.94 12l-1.72 1.72a.75.75 0 1 0 1.06 1.06L12 13.06l1.72 1.72a.75.75 0 1 0 1.06-1.06L13.06 12l1.72-1.72a.75.75 0 1 0-1.06-1.06L12 10.94l-1.72-1.72Z"
                clipRule="evenodd"
            />
        </svg>
    );
}

export function CheckCircleIcon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
            <path
                fillRule="evenodd"
                d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm13.36-1.814a.75.75 0 1 0-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 0 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.14-.094l3.75-5.25Z"
                clipRule="evenodd"
            />
        </svg>
    );
}

export function SpinnerIcon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="8.08 8.03 33.87 33.75">
            <path d="M41.9 23.9c-.3-6.1-4-11.8-9.5-14.4-6-2.7-13.3-1.6-18.3 2.6-4.8 4-7 10.5-5.6 16.6 1.3 6 6 10.9 11.9 12.5 7.1 2 13.6-1.4 17.6-7.2-3.6 4.8-9.1 8-15.2 6.9-6.1-1.1-11.1-5.7-12.5-11.7-1.5-6.4 1.5-13.1 7.2-16.4 5.9-3.4 14.2-2.1 18.1 3.7 1 1.4 1.7 3.1 2 4.8.3 1.4.2 2.9.4 4.3.2 1.3 1.3 3 2.8 2.1 1.3-.8 1.2-2.5 1.1-3.8 0-.4.1.7 0 0z"></path>
        </svg>
    );
}

export function ChevronDownIcon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
            <path
                fillRule="evenodd"
                d="M12.53 16.28a.75.75 0 0 1-1.06 0l-7.5-7.5a.75.75 0 0 1 1.06-1.06L12 14.69l6.97-6.97a.75.75 0 1 1 1.06 1.06l-7.5 7.5Z"
                clipRule="evenodd"
            />
        </svg>
    );
}

export function ChevronRightIcon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
            <path
                fillRule="evenodd"
                d="M16.28 11.47a.75.75 0 0 1 0 1.06l-7.5 7.5a.75.75 0 0 1-1.06-1.06L14.69 12 7.72 5.03a.75.75 0 0 1 1.06-1.06l7.5 7.5Z"
                clipRule="evenodd"
            />
        </svg>
    );
}

export function ArrowLongLeftIcon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
            <path
                fillRule="evenodd"
                d="M7.28 7.72a.75.75 0 0 1 0 1.06l-2.47 2.47H21a.75.75 0 0 1 0 1.5H4.81l2.47 2.47a.75.75 0 1 1-1.06 1.06l-3.75-3.75a.75.75 0 0 1 0-1.06l3.75-3.75a.75.75 0 0 1 1.06 0Z"
                clipRule="evenodd"
            />
        </svg>
    );
}

export function ArrowLongRightIcon(props: { className?: string }) {
    return (
        <svg className={props.className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
            <path
                fillRule="evenodd"
                d="M16.72 7.72a.75.75 0 0 1 1.06 0l3.75 3.75a.75.75 0 0 1 0 1.06l-3.75 3.75a.75.75 0 1 1-1.06-1.06l2.47-2.47H3a.75.75 0 0 1 0-1.5h16.19l-2.47-2.47a.75.75 0 0 1 0-1.06Z"
                clipRule="evenodd"
            />
        </svg>
    );
}
